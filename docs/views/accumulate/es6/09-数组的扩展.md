...
上面代码中，querySelectorAll 方法返回的是一个 NodeList 对象。它不是数组，而是一个类似数组的对象。这时，扩展运算符可以将其转为真正的数组，原因就在于 NodeList对象实现了 Iterator 。
...
fill 方法还可以 __接受第二个和第三个参数，用于指定填充的起始位置和结束位置__。
